...
function normalizeAnalysis(raw, fallbackText) {
    return {
        ...
        takeProfits: raw.take_profit_targets?.filter((target) => typeof target === 'string' && target.trim().length > 0) ||
            ['Monitor nearby liquidity and opposing structure for exits.'],
        confidence: extractConfidenceScore(raw.confidence_score),
        analysisText: raw.analysis_summary?.trim() ||
            fallbackText ||
        ...
        waitConditions: raw.wait_conditions?.trim() ||
            'Wait for confirmation at a key structure zone before entering.',
    };
}
async function analyzeChart(imagePath, pair, timeframe) {
