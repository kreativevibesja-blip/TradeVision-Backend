...
exports.analyzeChartVision = analyzeChartVision;
...
const buildOutputPaths = (filePath) => {
    const parsed = path_1.default.parse(filePath);
    return {
        normalizedPath: path_1.default.join(parsed.dir, `${parsed.name}_normalized.png`),
        edgePath: path_1.default.join(parsed.dir, `${parsed.name}_edges.png`),
    };
};
const toPrice = (y, pair) => {
    const template = (0, volatilityDetector_1.getPairTemplate)(pair);
    const normalized = 1 - y;
    return (0, volatilityDetector_1.roundPrice)(template.referencePrice + (normalized - 0.5) * template.amplitude, pair);
};
const detectRangeState = (variance, edgeDensity) => {
    if (variance > 0.06 || edgeDensity > 0.42)
        return 'expansion';
    if (variance < 0.022 && edgeDensity < 0.22)
        return 'compression';
    return 'balanced';
};
...
const detectSwings = (series, pair) => {
    const swings = [];
...
const pickLevels = (swings, type) => swings
    .filter((swing) => swing.type === type)
    .map((swing) => swing.price)
    .slice(-3);
const detectPatterns = (rangeState, trendStrength, edgeDensity) => {
    const patterns = [];
    if (rangeState === 'expansion')
        patterns.push('impulsive expansion candles');
    if (rangeState === 'compression')
        patterns.push('tight consolidation cluster');
    if (trendStrength > 0.5)
        patterns.push('trend continuation leg');
    if (edgeDensity > 0.35)
        patterns.push('high participation move');
    if (patterns.length === 0)
        patterns.push('balanced rotation candles');
    return patterns;
};
async function analyzeChartVision(filePath, pair) {
    const { normalizedPath, edgePath } = buildOutputPaths(filePath);
    const template = (0, volatilityDetector_1.getPairTemplate)(pair);
    await promises_1.default.mkdir(path_1.default.dirname(normalizedPath), { recursive: true });
    const normalizedPipeline = (0, sharp_1.default)(filePath)
    ...
    const stats = await (0, sharp_1.default)(normalizedPath).stats();
    ...
    const recentHigh = (0, volatilityDetector_1.roundPrice)(Math.max(...mappedPrices, ...(swingHighs.length ? swingHighs : [template.referencePrice + template.amplitude * 0.2])), pair);
    const recentLow = (0, volatilityDetector_1.roundPrice)(Math.min(...mappedPrices, ...(swingLows.length ? swingLows : [template.referencePrice - template.amplitude * 0.2])), pair);
    ...
    const volatility = (0, volatilityDetector_1.classifyVolatility)({
        pair,
        priceSpan,
    ...
    const demandStart = (0, volatilityDetector_1.roundPrice)(recentLow + priceSpan * 0.08, pair);
    const demandEnd = (0, volatilityDetector_1.roundPrice)(recentLow + priceSpan * 0.16, pair);
    const supplyStart = (0, volatilityDetector_1.roundPrice)(recentHigh - priceSpan * 0.16, pair);
    const supplyEnd = (0, volatilityDetector_1.roundPrice)(recentHigh - priceSpan * 0.08, pair);
    return {
        trend,
        recentHigh,
        recentLow,
        ...
        normalizedImageUrl: `/uploads/${path_1.default.basename(normalizedPath)}`,
        edgeImageUrl: `/uploads/${path_1.default.basename(edgePath)}`,
        imageStats: {
            width,
            height,
...
//# sourceMappingURL=chartVision.js.map