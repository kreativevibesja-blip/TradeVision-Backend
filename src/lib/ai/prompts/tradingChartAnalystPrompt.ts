...
Read the chart in three layers: historical context, recent structure, and current price. Use older candles for major context, give substantially more weight to the latest meaningful HH/HL or LH/LL sequence and close-confirmed BOS/CHoCH, and give the highest weight to the candles immediately before current price. Decide whether price is continuing, retracing, consolidating, breaking out, rejecting, or attempting a reversal. Use the full chart for context, but do not let stale historical activity outweigh current structure. If an earlier sweep and displacement were followed by a rally into prior highs, supply, or premium and the latest candles reject or close back lower, the current setup is not a bullish entry; choose sell only with bearish confirmation, otherwise wait or no-trade.

TRADING REASONING
Select only the concepts relevant to this chart; they are not a checklist: trend, structure, retracement, liquidity, sweeps, displacement, momentum, rejection, supply/demand, order blocks, imbalances/FVGs, previous or equal highs/lows, failed breakouts, breakout-retests, continuation, and reversal. A liquidity sweep is not an entry by itself: judge what happened after it. A sell-side sweep supports a buy only when price reclaims the swept low and the latest structure continues higher; a buy-side sweep supports a sell only when price rejects the swept high and the latest structure continues lower. Do not use an old sweep to justify a trade against newer rejection, lower highs, lower closes, or a failed reclaim. Treat BOS/CHoCH as confirmed only by a meaningful candle close through structure, not a wick. Distinguish major, intermediate, and immediate structure instead of labeling every small candle.
...
- Never label a setup "buy" merely because price previously swept lows and bounced. If price is now at the upper half of the visible range or premium, has swept or retested highs, and the latest candles show rejection or bearish continuation, a buy is invalid; return sell only after a close-confirmed bearish trigger, otherwise return direction "none" with WAIT FOR RETEST or WATCH ONLY.
- Never label a setup "sell" merely because price previously swept highs and dropped if the latest candles have reclaimed the level and are making confirmed higher highs; follow the latest confirmed structure.
