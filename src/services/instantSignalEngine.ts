...
const getExpiryMinutes = (assetClass: InstantSignalAssetClass, timeframe: string, status: InstantSignalStatus, confirmations: number) => {
  if (status === 'wait_confirmation') {
    const timeframeMinutes = timeframe.toLowerCase().includes('1m') || timeframe === 'M1' ? 1
      : timeframe.toLowerCase().includes('5m') || timeframe === 'M5' ? 5
      : timeframe.toLowerCase().includes('15m') || timeframe === 'M15' ? 15
      : 30;
    return Math.max(5, timeframeMinutes * confirmations + 2);
  }
...
export function generateInstantSignal(input: InstantSignalEngineInput): InstantSignalEngineOutput {
  ...
  if (contradiction || riskReward < 1.5 || confidence < 65 || choppy) {
    return noSignal(clamp(confidence, 35, 64), choppy ? 'Market is too choppy for a clean instant signal.' : 'Risk reward or directional confluence is not clean enough.');
  }

  const entryNow = confidence >= 75 && ((direction === 'buy' && sweptLow && bullishReaction) || (direction === 'sell' && sweptHigh && bearishReaction));
  const confirmationRequired = entryNow ? 0 : confidence >= 72 ? 1 : 2;
  const status: InstantSignalStatus = entryNow ? 'entry_now' : 'wait_confirmation';
  const expiresAt = new Date(Date.now() + getExpiryMinutes(input.assetClass, input.timeframe, status, confirmationRequired) * 60 * 1000).toISOString();
  ...
  return {
    ...
    confirmationText: entryNow ? null : `Wait for ${confirmationRequired} candle${confirmationRequired === 1 ? '' : 's'} to close ${direction === 'buy' ? 'bullish above the reaction level' : 'bearish below the reaction level'}.`,
    expiresAt,
  };
}
